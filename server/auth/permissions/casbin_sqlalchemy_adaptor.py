...
class Adapter(persist.Adapter, persist.adapters.UpdateAdapter):
    ...
    def load_policy(self, model):
        ...
        with self._session_scope() as session:
            """
            NOTE: `self._db_class.workspace_id == None` must use `==`
            cannot change to `self._db_class.workspace_id is None`, idk why
            """
            lines = (
                session.query(self._db_class).filter(self._db_class.workspace_id == None).all()  # noqa
            )
            for line in lines:
