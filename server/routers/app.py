...
@router.post("/")
def create_app_req(req: CreateAppRequest, router: DropbaseRouter = Depends(get_dropbase_router)):
...
def rename_app_req(req: RenameAppRequest):
    r_path_to_workspace = os.path.join(os.path.dirname(__file__), "../../workspace")
    workspace_folder_controller = WorkspaceFolderController(r_path_to_workspace)
