...
class PythonLSPServer(MethodDispatcher):
    ...
    def m_text_document__did_open(self, textDocument=None, **_kwargs):
        # print('m_text_document__did_open')
        # print(textDocument)
        workspace = self._match_uri_to_workspace(textDocument["uri"])
        # print(type(workspace))
        # print(workspace._root_path)
    ...
    def m_workspace__set_table_schema(self, dataclass: str = "", **_kwargs):
        # TODO: might not need this, generate context and definition schemas after create/rename/edit components
        from workspace.app.page1 import State, Context
        state_str = '\n\n\n'.join(generate_model_code(State))
        # print(state_str)

        # uri = 'file:///Users/ayazhan/dropbase/worker/workspace/app/page1/context.py'
        uri = 'file:///Users/ayazhan/dropbase/worker/workspace/app/page1/functions/function1.py'
        workspace = self._match_uri_to_workspace(uri)
        # print(type(workspace))
        # print(workspace._root_uri)
        workspace.put_document(uri, state_str, 1)

        self._hook("pylsp_document_did_open", uri)
...
def generate_model_code(model):
    ...
    for name, field in model.__fields__.items():
        if issubclass(field.outer_type_, BaseModel):
            nested_code = generate_model_code(field.outer_type_)
            default_value = field.default if field.default != ... else '...'
            # field_str = f"    {name} = {generate_field_code(default_value, field.outer_type_)}"
            field_str = f"    {name}: {field.outer_type_.__name__}"
            fields.append(field_str)
            models.extend(nested_code)
            # fields.append('\n' + nested_code)
        else:
            default_value = field.default if field.default != ... else '...'
            fields.append(f"    {name}: {field.outer_type_.__name__} = {default_value}")

    fields_str = ',\n'.join(fields)
    base_model_str = ', '.join([base.__name__ for base in model.__bases__])
    models.append(f"class {model_name}({base_model_str}):\n" + f"{fields_str}")
    return models
